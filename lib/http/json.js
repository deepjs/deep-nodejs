...
"use strict";
	...
	require("deepjs/lib/stores/http");
	require("deepjs/lib/stores/cache");
	var url = require('url');
	...
	deep.client.node = {};
	deep.client.node.JSONClient = deep.compose.Classes(client, deep.client.ClientStore, function(protocol, baseURI, schema, options){
		options = options || {};
		options.cache = (options.cache === false)?false:true;
		if(options.cache)
			deep.sheet(deep.mediaCache.sheets.store, this);
	});
