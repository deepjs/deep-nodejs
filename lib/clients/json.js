...
"use strict";
	...
	var httpCall = function(uri, datas, method, options)
	{
		...
		var req = http.request(infos, function(res) {
			...
			{
				er = e;
				console.log("deep-http-client/node/json : error : ", e);
				if(!def.rejected)
					def.reject(e);
			});
		});
		req.on('error', function(e) {
			console.log("json req : on error : ", e);
			def.reject(e);
		});
		if(datas)
		...
		range:function(uri, options)
		{
			...
			.done(function(data){
				var res = {
					...
					data:self.responseParser(data.body, msg, jqXHR)
				};
				def.resolve(res);
			})
			.fail(function(e){
				def.reject(deep.errors.Error(e.status || 400, e.msg));
			});
		},
	...
	deep.client.node = {};
	deep.client.node.JSONClient = deep.compose.Classes(client, deep.client.ClientStore);
	deep.client.node.JSONClient.create = function(protocol, baseURI, schema, options){
		var client = new deep.client.node.JSONClient(protocol, baseURI, schema, options);
		if(protocol)
            deep.utils.up(deep.protocol.SheetProtocoles, client);
        return client;
	};
	deep.client.node.JSONClient.createDefault = function(){
		var client = new deep.client.node.JSONClient("json", "");
        deep.utils.up(deep.protocol.SheetProtocoles, client);
        return client;
	};
